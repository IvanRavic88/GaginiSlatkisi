    ...
    <>
      ...
      {sweets.length > 0 ? (
        <ProductJsonLd
          sweets={sweets
            .filter(
              (s): s is typeof s & { name: string; description: string; image: SanityImageType } =>
                Boolean(s.name && s.description && s.image),
            )
            .map((s) => ({
              name: s.name,
      ...
      <Container as="section" className="py-[6.4rem] text-center">
        ...
        {sweets.length === 0 ? (
          <div className="mx-auto mt-[6.4rem] max-w-[56rem] rounded-[16px] border border-[var(--color-primary-shade)]/20 bg-white px-[3.2rem] py-[6.4rem] shadow-[0_1.6rem_3.2rem_rgba(0,0,0,0.05)]">
            <p className="mb-[2.4rem] font-[family-name:var(--font-caveat)] text-[4.2rem] leading-none font-bold text-[var(--color-primary-shade)]">
              Uskoro stiže…
            </p>
